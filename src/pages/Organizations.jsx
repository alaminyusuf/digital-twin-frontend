...
const Organizations = () => {
	...
	const onAddMember = async (e) => {
		...
		setLoading(true)
		try {
			await api.post(`/api/organizations/${activeOrg}/members`, { 
				email: memberEmail, 
				role: memberRole 
			})
			setMemberEmail('')
	...
	const onRemoveMember = async (memberId) => {
		...
		setLoading(true)
		try {
			await api.delete(`/api/organizations/${activeOrg}/members/${memberId}`)
			await fetchOrganizations()
			setLoading(false)
		} catch (err) {
	...
	const onUpdateRole = async (memberId, newRole) => {
		setLoading(true)
		try {
			await api.put(`/api/organizations/${activeOrg}/members/${memberId}/role`, { 
				role: newRole 
			})
			await fetchOrganizations()
			setLoading(false)
